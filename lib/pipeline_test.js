...
export function do_pipeline(canvas, from_fn) {
    console.log("computing sample asyncronously");
    (async () => await do_pipeline_async(canvas, from_fn) )();
};
...
async function do_pipeline_async(canvas, from_fn) {
    debugger;
    ...
    const output_shape = [K * 2, I * 2];//shape_in.slice(1);
...
export function do_rotation(roll, pitch, yaw) {
    ...
    const translate_out = qdVector.affine3d(null, [-K, -I, -1.1*K]);
