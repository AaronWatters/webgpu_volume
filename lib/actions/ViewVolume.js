...
export class View extends GPUAction.Action {
    ...
    pick_on(canvas, callback, etype) {
        etype = etype || 'click';
        this.canvas_space = new coordinates.NormalizedCanvasSpace(canvas);
        const [width, height] = this.output_shape;
        this.panel_space = new coordinates.PanelSpace(width, height, this.canvas_space);
        const that = this;
        canvas.addEventListener(etype, async function(event) {
            const pick = await that.pick(event);
