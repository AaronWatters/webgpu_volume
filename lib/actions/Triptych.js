...
export class Triptych extends viewVolume.View {
    ...
    async set_colorize(colorize_checked) {
        if (colorize_checked) {
            this.colorize = 1;
        } else {
            this.colorize = 0;
        }
        await this.max_gray_action.change_colorize(this.colorize);
        await this.slice_gray_action.change_colorize(this.colorize);
        this.run();
    };
    async change_volume(intensityCPUBuffer) {
        //debugger;
        this.ofVolume.set_data(intensityCPUBuffer);
        this.ofVolume.push_buffer();
        this.min_value = this.ofVolume.min_value;
        this.max_value = this.ofVolume.max_value;
        //await this.max_gray_action.change_range(this.min_value, this.max_value);
        //await this.slice_gray_action.change_range(this.min_value, this.max_value);
        this.run();
    };
    ...
    panel_sequence(context) {
        ...
        let qd_colorize = this.colorize;;
        debugger;
        this.max_gray_action = context.to_gray_panel(
            this.max_panel, this.max_gray_panel, this.min_value, this.max_value, qd_colorize);
        ...
        this.slice_gray_action = context.to_gray_panel(
            this.slice_value_panel, this.slice_gray_panel, this.min_value, this.max_value, qd_colorize);
