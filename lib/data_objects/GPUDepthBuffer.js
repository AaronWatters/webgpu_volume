...
export class DepthBuffer extends GPUDataObject.DataObject {
    ...
    reset_to_defaults() {
        // reset data and depths to default values.
        console.log("resetting depth buffer.")
        this.data = new this.data_format(this.size);
        this.depths = new Float32Array(this.size);
        for (let i = 0; i < this.size; i++) {
            this.data[i] = this.default_value;
            this.depths[i] = this.default_depth;
        }
        // push to GPU
        this.push_buffer();
    };
    ...
    location([row, column], projection_space, in_volume) {
        ...
        if (projection_space) {
            ...
            if (in_volume) {
                ...
                result.volume_ijk = int_ijk; // convert to integers, always ???
