...
export class OutlinePipeline {
    ...
    async load() {
        ...
        // joint project action from volume to depth buffer
        ...
        // flatten skeleton depths onto panel
        //this.skeleton_panel = context.panel(width, height);
        this.skeleton_flatten = this.skeleton_depth_buffer.flatten_action(this.outline_panel);
    ...
    change_parameters(affine_transform, ratio) {
        if (affine_transform){
            ...
            this.joint_project.change_matrix(M);
        }
        if (ratio) {
