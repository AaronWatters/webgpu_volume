...
export class Orbiter {
    ...
    pointerdown(e) {
        ...
        //l("pointerdown at", this.last_stats);
        //l("  current rotation", this.current_rotation);
    };
    pointermove(e) {
        if (!this.active) {
            return;
    ...
    pointerup(e) {
        ...
        //l("pointerup at", this.next_stats);
        //l("  current rotation", this.current_rotation);
    };
    do_rotation(e) {
        //debugger;
        ...
        //const offset_x = clamp1(scale * (now.dx - last.dx));
        //const offset_y = clamp1(scale * (now.dy - last.dy));
        //const yaw = Math.asin(offset_x);
        //const pitch = Math.asin(offset_y);
        const ascale = Math.PI / 2.0;
        const yaw = ascale * (offset_x);
        const pitch = ascale * (offset_y);
        //l("offsetx", offset_x, "offset_y", offset_y)
    ...
    event_stats(e) {
        ...
        const px = e.pageX;
        const py = e.pageY;
        const cx = brec.width/2 + brec.left; // xxx constant
        const cy = brec.height/2 + brec.top; // xxx constant
        const offsetx = (px - cx);
        const offsety = - (py - cy);
